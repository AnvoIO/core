...
namespace core_net::testing {
   ...
   struct signed_transaction_w_signer {
      signed_transaction_w_signer(core_net::chain::signed_transaction trx, fc::crypto::private_key key) : _trx(std::move(trx)), _signer(key) {}

      core_net::chain::signed_transaction _trx;
      fc::crypto::private_key _signer;
   };

   struct action_pair_w_keys {
      action_pair_w_keys(core_net::chain::action first_action, core_net::chain::action second_action, fc::crypto::private_key first_act_signer, fc::crypto::private_key second_act_signer)
      ...
      core_net::chain::action _first_act;
      core_net::chain::action _second_act;
   ...
   struct trx_generator_base_config {
      ...
      core_net::chain::chain_id_type _chain_id = core_net::chain::chain_id_type::empty_chain_id();
      core_net::chain::name _contract_owner_account = core_net::chain::name();
      fc::microseconds _trx_expiration_us = fc::seconds(3600);
      core_net::chain::block_id_type _last_irr_block_id = core_net::chain::block_id_type();
      std::string _log_dir = ".";
   ...
   struct accounts_config {
      std::vector<core_net::chain::name> _acct_name_vec;
   ...
   struct trx_generator_base {
      ...
      virtual void update_resign_transaction(core_net::chain::signed_transaction& trx, const fc::crypto::private_key& priv_key, uint64_t& nonce_prefix, uint64_t& nonce,
                                     const fc::microseconds& trx_expiration, const core_net::chain::chain_id_type& chain_id, const core_net::chain::block_id_type& last_irr_block_id);

      void push_transaction(signed_transaction_w_signer& trx, uint64_t& nonce_prefix,
                            uint64_t& nonce, const fc::microseconds& trx_expiration, const core_net::chain::chain_id_type& chain_id,
                            const core_net::chain::block_id_type& last_irr_block_id);

      void set_transaction_headers(core_net::chain::transaction& trx, const core_net::chain::block_id_type& last_irr_block_id, const fc::microseconds& expiration, uint32_t delay_sec = 0);

      signed_transaction_w_signer create_trx_w_actions_and_signer(std::vector<core_net::chain::action>&& act, const fc::crypto::private_key& priv_key, uint64_t& nonce_prefix,
                                                                  uint64_t& nonce, const fc::microseconds& trx_expiration, const core_net::chain::chain_id_type& chain_id,
                                                                  const core_net::chain::block_id_type& last_irr_block_id);

      void log_first_trx(const std::string& log_dir, const core_net::chain::signed_transaction& trx);
   ...
   struct transfer_trx_generator : public trx_generator_base {
      ...
      core_net::chain::bytes make_transfer_data(const core_net::chain::name& from, const core_net::chain::name& to, const core_net::chain::asset& quantity, const std::string& memo);
      auto make_transfer_action(core_net::chain::name account, core_net::chain::name from, core_net::chain::name to, core_net::chain::asset quantity, std::string memo);
   ...
   struct trx_generator : public trx_generator_base{
      ...
      core_net::chain::abi_serializer _abi;
      ...
      std::vector<core_net::chain::action> generate_actions();
      virtual void update_resign_transaction(core_net::chain::signed_transaction& trx, const fc::crypto::private_key& priv_key, uint64_t& nonce_prefix, uint64_t& nonce,
                                     const fc::microseconds& trx_expiration, const core_net::chain::chain_id_type& chain_id, const core_net::chain::block_id_type& last_irr_block_id);
