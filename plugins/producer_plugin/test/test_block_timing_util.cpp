...
static_assert(core_net::chain::config::block_interval_ms == 500);

constexpr auto       block_interval = fc::microseconds(core_net::chain::config::block_interval_us);
constexpr auto       cpu_effort_us  = 400000;
constexpr auto       cpu_effort     = fc::microseconds(cpu_effort_us);
constexpr auto       production_round_1st_block_slot = 100 * core_net::chain::config::producer_repetitions;
...
BOOST_AUTO_TEST_CASE(test_production_round_block_start_time) {
   const fc::time_point production_round_1st_block_time =
      core_net::chain::block_timestamp_type(production_round_1st_block_slot).to_time_point();
   auto expected_start_time = production_round_1st_block_time - block_interval;
   for (int i = 0; i < core_net::chain::config::producer_repetitions;
        ++i, expected_start_time = expected_start_time + cpu_effort) {
      auto block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + i);
      BOOST_CHECK_EQUAL(core_net::block_timing_util::production_round_block_start_time(cpu_effort, block_time), expected_start_time);
   }
}

BOOST_AUTO_TEST_CASE(test_calculate_block_deadline) {
   using namespace core_net::block_timing_util;
   const fc::time_point production_round_1st_block_time =
      core_net::chain::block_timestamp_type(production_round_1st_block_slot).to_time_point();

   {
      ...
      for (int i = 0; i < core_net::chain::config::producer_repetitions; ++i) {
         auto block_time        = core_net::chain::block_timestamp_type(production_round_1st_block_slot + i);
   ...
   }
   {
      ...
      auto second_block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + 1);
      ...
      auto third_block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + 2);
      ...
      auto forth_block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + 3);
      ...
      auto seventh_block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + 6);
      ...
      auto eighth_block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + 7);
      ...
      auto ninth_block_time = core_net::chain::block_timestamp_type(production_round_1st_block_slot + 8);
   ...
   }
   {
      ...
      auto first_block_time = core_net::chain::block_timestamp_type(fc::time_point::from_iso_string("2023-10-29T13:40:30.000"));
      ...
      auto second_block_time = core_net::chain::block_timestamp_type(fc::time_point::from_iso_string("2023-10-29T13:40:30.500"));
   ...
   }
   {
      ...
      auto default_cpu_effort = fc::microseconds(block_interval.count() - (450000/12)); // 462,500
      auto block_time = core_net::chain::block_timestamp_type(fc::time_point::from_iso_string("2023-10-31T13:37:35.000"));
...
BOOST_AUTO_TEST_CASE(test_calculate_producer_wake_up_time) {
   ...
   constexpr fc::microseconds full_cpu_effort = fc::microseconds{core_net::chain::config::block_interval_us};
   ...
   { // Only one producer in active_schedule, we should produce every block
      ...
      const uint32_t prod_round_1st_block_slot = 100 * active_schedule.size() * core_net::chain::config::producer_repetitions - 1;
   ...
   { // Tests for when we only have a subset of all active producers, we do not produce all blocks, only produce blocks for our round
      ...
      constexpr fc::microseconds half_cpu_effort = fc::microseconds{core_net::chain::config::block_interval_us / 2u};
      producers = std::set<account_name>{ "initc"_n };
      block_timestamp = block_timestamp_type(prod_round_1st_block_slot);
      expected_block_time = block_timestamp_type(prod_round_1st_block_slot + 2*config::producer_repetitions).to_time_point();
   ...
   { // actual example that caused multiple start blocks
      ...
      auto default_cpu_effort = fc::microseconds(block_interval.count() - (450000/12)); // 462,500
      auto wake_time = calculate_producer_wake_up_time(default_cpu_effort, 106022362, core_net::chain::block_timestamp_type(fc::time_point::from_iso_string("2023-10-31T16:06:41.000")),
