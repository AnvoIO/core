...
namespace {
   ...
   void emit_killer(F&& f) {
      try {
         ...
         CORE_THROW(chain::controller_emit_signal_exception, "Trace API encountered an Error which it cannot recover from.  Please resolve the error and relaunch the process")
      }
   }
...
struct trace_api_common_impl {
   ...
   void plugin_initialize(const appbase::variables_map& options) {
      ...
      CORE_ASSERT(blocks >= -1, chain::plugin_config_exception,
      ...
      CORE_ASSERT(uncompressed_blocks >= -1, chain::plugin_config_exception,
...
struct trace_api_rpc_plugin_impl : public std::enable_shared_from_this<trace_api_rpc_plugin_impl>
{
   ...
   void plugin_initialize(const appbase::variables_map& options) {
      ...
      if( options.count("trace-rpc-abi") ) {
         CORE_ASSERT(options.count("trace-no-abis") == 0, chain::plugin_config_exception,
