...
BOOST_AUTO_TEST_SUITE(corevmoc_interrupt_tests)
...
BOOST_AUTO_TEST_CASE( wasm_interrupt_test ) { try {
   ...
   savanna_validating_tester t(
      ...
      [&](controller::config& cfg) {
         cfg.core_vm_oc_whitelist_suffixes.insert("testapi"_n);
         if (cfg.wasm_runtime != wasm_interface::vm_type::core_vm_oc)
            cfg.corevmoc_tierup = chain::wasm_interface::vm_oc_enable::oc_auto;
      },
      use_genesis
   );
   ...
   // corevmoc. That action will then fail when it hits the 5000ms deadline.
   ...
   auto post_count = t.control->get_wasm_interface().get_core_vm_oc_compile_interrupt_count();
