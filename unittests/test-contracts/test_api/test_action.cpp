...
void test_action::test_dummy_action() {
   ...
   eosio_assert( core_net::pack_size(act) == static_cast<size_t>(total), "pack_size does not match get_action size" );
...
void test_action::test_cf_action() {
   ...
   if ( cfa.payload == 100 ) {
      ...
      eosio_assert(core_net::transaction_size() > 0, "transaction_size failed");
