...
proxy::proxy( core_net::name self, core_net::name first_receiver, core_net::datastream<const char*> ds )
...
void proxy::on_transfer( name from, name to, asset quantity, const std::string& memo ) {
   ...
   if( from == self ) {
      ...
      transaction out;
      core_net::token::transfer_action a( "eosio.token"_n, {self, "active"_n} );
...
void proxy::on_error( uint128_t sender_id, core_net::ignore<std::vector<char>> ) {
