...
struct act_sig {
   core_net::chain::signature_type sig;
...
auto get_resolver(const abi_def& abi = abi_def())
{
   return [&abi](const account_name &name) -> std::optional<abi_serializer> {
      return abi_serializer(core_net_contract_abi(abi), yield_fn());
   };
}
...
{ try {
   ...
   abi_serializer abis(core_net_contract_abi(abi), yield_fn());
...
{ try {
   ...
   // ------------------------------------------------------------------------------------------
   BOOST_CHECK_EXCEPTION( abis.set_abi(core_net_contract_abi(abi), yield_fn()), invalid_type_inside_abi, log_exception );
...
{ try {
   ...
   auto abi = core_net_contract_abi(fc::json::from_string(my_abi).as<abi_def>());
...
{ try {
   ...
   auto abi = core_net_contract_abi(fc::json::from_string(typedef_cycle_abi).as<abi_def>());
...
{ try {
   ...
   abi_serializer abis(core_net_contract_abi(abi_def()), yield_fn());
...
{ try {
   ...
   BOOST_TEST("PUB_K1_129oZmmV2ZZrkNrcf5T6CZD6nxm6EDztj6h8pP563wWG7oym29" == updauth.auth.keys[0].key.to_string({}));
...
{ try {
   ...
   BOOST_TEST("PUB_K1_129oZmmV2ZZrkNrcf5T6CZD6nxm6EDztj6h8pP563wWG7oym29" == newacct.owner.keys[0].key.to_string({}));
   ...
   BOOST_TEST("PUB_K1_129oZmmV2ZZrkNrcf5T6CZD6nxm6EDztj6h8pP563wWG7oym29" == newacct.active.keys[0].key.to_string({}));
...
{ try {
   ...
   abi_serializer abis(core_net_contract_abi(v.as<abi_def>()), yield_fn());
...
{ try {
   ...
   auto abi = core_net_contract_abi(fc::json::from_string(repeat_abi).as<abi_def>());
...
{
   using core_net::testing::fc_exception_message_starts_with;
...
BOOST_AUTO_TEST_CASE(version)
{
   try {
      ...
      BOOST_CHECK_THROW( abi_serializer(fc::json::from_string(R"({"version": "core_net::abi/9.0"})").as<abi_def>(), yield_fn()), unsupported_abi_version_exception );
      abi_serializer(fc::json::from_string(R"({"version": "core_net::abi/1.0"})").as<abi_def>(), yield_fn());
      abi_serializer(fc::json::from_string(R"({"version": "core_net::abi/1.1"})").as<abi_def>(), yield_fn());
   } FC_LOG_AND_RETHROW()
}
...
{
   using core_net::testing::fc_exception_message_is;
...
inline std::pair<action_trace, std::string> generate_action_trace(const std::optional<T> &  return_value, const std::string &  return_value_hex, bool parsable = true)
{
   ...
   at.act = core_net::chain::action(
      std::vector<core_net::chain::permission_level>{
         core_net::chain::permission_level{
...
BOOST_AUTO_TEST_CASE(abi_to_variant__add_action__good_return_value)
{
...

   {
      ...
      core_net::chain::impl::abi_traverse_context ctx(yield_fn(), fc::microseconds{});
      core_net::chain::impl::abi_to_variant::add(mvo, "action_traces", at, get_resolver(abidef), ctx);
   ...
   {
      mutable_variant_object mvo;
      core_net::chain::impl::abi_traverse_context ctx(abi_serializer::create_depth_yield_function(), max_serialization_time);
      core_net::chain::impl::abi_to_variant::add(mvo, "action_traces", at, get_resolver(abidef), ctx);
      std::string res = fc::json::to_string(mvo, get_deadline());
