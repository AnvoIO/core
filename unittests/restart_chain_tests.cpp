...
struct dummy_action {
   static core_net::chain::name get_name() { return "dummyaction"_n; }
   static core_net::chain::name get_account() { return "testapi"_n; }
...
struct cf_action {
   static core_net::chain::name get_name() { return "cfaction"_n; }
   static core_net::chain::name get_account() { return "testapi"_n; }
...
BOOST_AUTO_TEST_CASE_TEMPLATE( test_restart_with_different_chain_id, T, testers ) {
   ...
   genesis.initial_key       = core_net::testing::base_tester::get_public_key(config::system_account_name, "active");
