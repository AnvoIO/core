...
#define CORE_NET_VM_OPEN_NAMESPACE namespace core_net::vm {
#define CORE_NET_VM_CLOSE_NAMESPACE }

#include "error_codes_pp.hpp"

// create a category for parser error codes
GENERATE_ERROR_CATEGORY(parser_errors, "core-vm parser errors")
GENERATE_ERROR_CATEGORY(memory_errors, "core-vm memory errors")
GENERATE_ERROR_CATEGORY(system_errors, "core-vm system errors")
GENERATE_ERROR_CATEGORY(auxiliary_errors, "core-vm auxiliary errors")
...
#define _CORE_NET_VM_ASSERT(expr, err_type)                                                                                  \
   if (!UNLIKELY(expr)) {                                                                                              \
      return err_type;                                                                                                 \
   }

// assert but invalidate the enclosing class
#define CORE_NET_VM_ASSERT_INVALIDATE(expr, err_type)                                                                       \
