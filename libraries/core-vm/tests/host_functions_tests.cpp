...
// wasm hex
/* Code used to generate test, compile with core_net-cpp v1.6.2 with minor manual edits to remove unneeded imports
 * extern "C" {
      struct state_t { float f; int i; };
      [[core_net::wasm_import]]
...
struct init_backend {
   ...
   using rhf_t     = core_net::vm::registered_host_functions<Host, execution_interface, cnv<Host>>;
   using backend_t = core_net::vm::backend<rhf_t, Impl>;
   wasm_allocator wa;
...
BACKEND_TEST_CASE( "Test C-style host function system", "[C-style_host_functions_tests]") {
   wasm_allocator wa;
   using rhf_t     = core_net::vm::registered_host_functions<standalone_function_t, execution_interface, cnv<standalone_function_t>>;
   using backend_t = core_net::vm::backend<rhf_t, TestType>;
