...
struct example_host_methods {
   ...
   void core_net_assert(bool test, const char* msg) {
      if (!test) {
...
struct cnv : type_converter<example_host_methods> {
   ...
   CORE_NET_VM_FROM_WASM(char*, (void* ptr)) { return static_cast<char*>(ptr); }
   CORE_NET_VM_FROM_WASM(const char*, (void* ptr)) { return static_cast<char*>(ptr); }
};

CORE_NET_VM_PRECONDITION(test_name,
      CORE_NET_VM_INVOKE_ON(const char*, [&](auto&& nm, auto&&... rest) {
         std::string s = nm;
         if (s == "core-vm2")
            throw "failure";
   }))
...
int main(int argc, char** argv) {
   ...
   using rhf_t     = core_net::vm::registered_host_functions<example_host_methods, execution_interface, cnv>;
   using backend_t = core_net::vm::backend<rhf_t>;

   // register print_num
   rhf_t::add<&example_host_methods::print_num>("env", "print_num");
   // register core_net_assert
   rhf_t::add<&example_host_methods::core_net_assert>("env", "core_net_assert");
   // register print_name
   ...
   } catch (...) { std::cerr << "core-vm interpreter error\n"; }
   return 0;
}
