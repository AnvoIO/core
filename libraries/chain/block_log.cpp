...
namespace core_net { namespace chain {
   ...
   struct block_log_preamble {
      ...
      void read_from(Stream& ds, const std::filesystem::path& log_path) {
         ...
         CORE_ASSERT(version() > 0, block_log_exception, "Block log was not setup properly");
         CORE_ASSERT(block_log::is_supported_version(version()), block_log_unsupported_version,
         ...
         if (block_log::contains_genesis_state(version(), first_block_num)) {
            ...
            CORE_THROW(block_log_exception,
         ...
         if (version() != initial_version) {
            ...
            CORE_ASSERT(actual_totem == expected_totem, block_log_exception,
   ...
   namespace {
      ...
      signed_block_ptr read_block(Stream&& ds, uint32_t expect_block_num = 0) {
         ...
         if (expect_block_num != 0) {
            CORE_ASSERT(!!block && block->block_num() == expect_block_num, block_log_exception,
      ...
      signed_block_header read_block_header(Stream&& ds, uint32_t expect_block_num) {
         ...
         CORE_ASSERT(bh.block_num() == expect_block_num, block_log_exception,
      ...
      class block_log_data : public chain::log_data_base<block_log_data> {
       ...
       public:
         ...
         uint32_t block_num_at(uint64_t position) {
            ...
            CORE_ASSERT(position + blknum_offset + sizeof(uint32_t) <= size(), block_log_exception,
         ...
         void light_validate_block_entry_at(uint64_t pos, uint32_t expected_block_num) {
            ...
            CORE_ASSERT(actual_block_num == expected_block_num, block_log_exception,
         ...
         std::tuple<uint32_t, block_id_type> full_validate_block_entry(uint32_t             previous_block_num,
            ...
            CORE_ASSERT(pos == tmp_pos, block_log_exception,
      ...
      struct block_log_bundle {
         ...
         block_log_bundle(std::filesystem::path block_file, std::filesystem::path index_file, bool validate_indx)
            ...
            CORE_ASSERT(!log_data.get_preamble().is_currently_pruned(), block_log_unsupported_version,
   ...
   struct block_log_verifier {
      ...
      void verify(block_log_data& log, const std::filesystem::path& log_path) {
         if (chain_id.empty()) {
            ...
            CORE_ASSERT(chain_id == log.chain_id(), block_log_exception,
   ...
   namespace detail {
      ...
      struct basic_block_log : block_log_impl {
         ...
         void append(const signed_block_ptr& b, const block_id_type& id,
            ...
            try {
               CORE_ASSERT(genesis_written_to_block_log, block_log_append_fail,
               ...
               CORE_ASSERT(index_file.tellp() == sizeof(uint64_t) * (b->block_num() - preamble.first_block_num),
         ...
         block_pos_size_t get_block_position_and_size(uint32_t block_num) {
            ...
            CORE_ASSERT(block_num <= last_block_num, block_log_exception,
            ...
            if (block_num < last_block_num) {
               ...
               CORE_ASSERT(next_block_pos > pos + block_pos_size, block_log_exception,
         ...
         void open(const std::filesystem::path& data_dir) {
            ...
            if (log_size) {
               ...
               CORE_ASSERT(index_size || number_of_blocks == 0, block_log_exception,
                          "${index_file} file is empty, please use core-util to fix the problem.",
                          ("index_file", index_file.get_file_path().string()));
               CORE_ASSERT(index_size % sizeof(uint64_t) == 0, block_log_exception,
               ...
               if (index_size) {
                  ...
                  CORE_ASSERT(last_block_pos == last_index_pos, block_log_exception,
         ...
         size_t convert_existing_header_to_vacuumed(uint32_t first_block_num) {
            ...
            CORE_ASSERT(is_pruned_log_and_mask_version(old_version), block_log_exception,
            ...
            if (block_log::contains_genesis_state(old_version, old_first_block_num)) {
               ...
               if (first_block_num == 1) {
                  CORE_ASSERT(old_first_block_num == 1, block_log_exception, "expected an old first blocknum of 1");
      ...
      struct partitioned_block_log final : basic_block_log {
         ...
         partitioned_block_log(const std::filesystem::path& log_dir, const partitioned_blocklog_config& config) : stride(config.stride) {
            ...
            if ((log_size == 0 || !head) && !catalog.empty()) {
               ...
               CORE_ASSERT(catalog.verifier.chain_id.empty() || catalog.verifier.chain_id == preamble.chain_id(),
         ...
         void reset(const chain_id_type& chain_id, uint32_t first_block_num) final {
            ...
            CORE_ASSERT(catalog.verifier.chain_id.empty() || chain_id == catalog.verifier.chain_id, block_log_exception,
      ...
      struct punch_hole_block_log final : basic_block_log {
         ...
         punch_hole_block_log(const std::filesystem::path& data_dir, const prune_blocklog_config& prune_conf)
            ...
            CORE_ASSERT(__builtin_popcount(prune_config.prune_threshold) == 1, block_log_exception,
   ...
   block_log_data::full_validate_blocks(uint32_t last_block_num, const std::filesystem::path& blocks_dir, fc::time_point now) {
      ...
      file.seek(pos);
      CORE_ASSERT(!is_currently_pruned(), block_log_exception, "pruned block log cannot be repaired");
      try {
         try {
            signed_block entry;
   ...
   std::filesystem::path block_log::repair_log(const std::filesystem::path& data_dir, uint32_t truncate_at_block,
      ...
      CORE_ASSERT(std::filesystem::is_directory(data_dir) && std::filesystem::is_regular_file(data_dir / "blocks.log"), block_log_not_found,
      ...
      CORE_ASSERT(!std::filesystem::exists(backup_dir), block_log_backup_dir_exist,
   ...
   bool block_log::trim_blocklog_front(const std::filesystem::path& block_dir, const std::filesystem::path& temp_dir,
      ...
      CORE_ASSERT(block_dir != temp_dir, block_log_exception, "block_dir and temp_dir need to be different directories");
   ...
   void block_log::extract_block_range(const std::filesystem::path& block_dir, const std::filesystem::path& dest_dir,
      ...
      CORE_ASSERT(start_block_num >= log_bundle.log_data.first_block_num(), block_log_exception,
