...
*/
   ...
   const char* builtin_protocol_feature_codename( builtin_protocol_feature_t codename ) {
      ...
      CORE_ASSERT( itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
   ...
   ,_type( feature_type )
   {
      switch( feature_type ) {
         ...
         default:
         {
            CORE_THROW( protocol_feature_validation_exception,
   ...
   {
      CORE_ASSERT( builtin_feature, protocol_feature_exception, "not a builtin protocol feature" );
   ...
   const protocol_feature& protocol_feature_set::get_protocol_feature( const digest_type& feature_digest )const {
      ...
      CORE_ASSERT( itr != _recognized_protocol_features.end(), protocol_feature_exception,
   ...
   const protocol_feature& protocol_feature_set::add_feature( const builtin_protocol_feature& f ) {
      ...
      CORE_ASSERT( builtin_itr != builtin_protocol_feature_codenames.end(), protocol_feature_validation_exception,
      ...
      if( indx < _recognized_builtin_protocol_features.size() ) {
         CORE_ASSERT( _recognized_builtin_protocol_features[indx] == _recognized_protocol_features.end(),
      ...
      if( expected_builtin_dependencies.size() > satisfied_builtin_dependencies.size() ) {
         ...
         for( const auto& builtin_codename : missing_builtins ) {
            ...
            CORE_ASSERT( itr != builtin_protocol_feature_codenames.end(),
                        protocol_feature_exception,
                        "Unexpected error"
            );
      ...
      CORE_ASSERT( res.second, protocol_feature_exception,
   ...
   void protocol_feature_manager::init( chainbase::database& db ) {
      CORE_ASSERT( !is_initialized(), protocol_feature_exception, "cannot initialize protocol_feature_manager twice" );
   ...
   const protocol_feature* protocol_feature_manager::const_iterator::get_pointer()const {
      //CORE_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot dereference singular iterator" );
      //CORE_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot dereference end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator++() {
      CORE_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot increment singular iterator" );
      CORE_ASSERT( _index != end_index, protocol_feature_iterator_exception, "cannot increment end iterator" );
   ...
   protocol_feature_manager::const_iterator& protocol_feature_manager::const_iterator::operator--() {
      CORE_ASSERT( _pfm, protocol_feature_iterator_exception, "cannot decrement singular iterator" );
      if( _index == end_index ) {
         CORE_ASSERT( _pfm->_activated_protocol_features.size() > 0,
                     protocol_feature_iterator_exception,
   ...
   {
      CORE_ASSERT( is_initialized(), protocol_feature_exception, "protocol_feature_manager is not yet initialized" );

      auto itr = _protocol_feature_set.find( feature_digest );

      CORE_ASSERT( itr != _protocol_feature_set.end(), protocol_feature_exception,
                  "unrecognized protocol feature digest: ${digest}", ("digest", feature_digest) );

      if( _activated_protocol_features.size() > 0 ) {
         const auto& last = _activated_protocol_features.back();
         CORE_ASSERT( last.activation_block_num <= current_block_num,
                     protocol_feature_exception,
      ...
      CORE_ASSERT( itr->builtin_feature,
                  protocol_feature_exception,
      ...
      CORE_ASSERT( indx < _builtin_protocol_features.size(), protocol_feature_exception,
      ...
      CORE_ASSERT( _builtin_protocol_features[indx].activation_block_num == builtin_protocol_feature_entry::not_active,
                  protocol_feature_exception,
   ...
   protocol_feature_set initialize_protocol_features( const std::filesystem::path& p, bool populate_missing_builtins ) {
      ...
      if( std::filesystem::exists( p ) ) {
         CORE_ASSERT( std::filesystem::is_directory( p ), plugin_exception,
      ...
      if( directory_exists ) {
         for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
            ...
            CORE_ASSERT( res.second, plugin_exception,
      ...
      auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
         ...
         CORE_ASSERT( !std::filesystem::exists( file_path ), plugin_exception,
