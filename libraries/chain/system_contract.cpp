...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      CORE_ASSERT( acct != nullptr, action_validate_exception,
      ...
      if( a.permission.permission == config::code_name() ) // virtual eosio.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         CORE_THROW( action_validate_exception,
...
void apply_eosio_newaccount(apply_context& context) {
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "newaccount not allowed in read-only transaction" );
   auto create = context.get_action().data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::auth_scope() );
   auto& authorization = context.control.get_mutable_authorization_manager();

   CORE_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   CORE_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   CORE_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   CORE_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_metadata_object, by_name>(create.creator);
   if( !creator.is_privileged() ) {
      CORE_ASSERT( name_str.find( config::system_account_prefix_str() ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with '${p}'",
                  ("p", config::system_account_prefix_str()) );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   CORE_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_eosio_setcode(apply_context& context) {
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "setcode not allowed in read-only transaction" );
   auto& db = context.db;
   auto  act = context.get_action().data_as<setcode>();
   context.require_authorization(act.account);

   CORE_ASSERT( act.vmtype == 0, invalid_contract_vm_type, "code should be 0" );
   CORE_ASSERT( act.vmversion == 0, invalid_contract_vm_version, "version should be 0" );
   ...
   CORE_ASSERT( code_size > 0 || existing_code, set_exact_code, "contract is already cleared" );
   ...
   if( existing_code ) {
      ...
      CORE_ASSERT( old_code_entry.code_hash != code_hash, set_exact_code,
...
void apply_eosio_setabi(apply_context& context) {
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "setabi ot allowed in read-only transaction" );
   auto& db  = context.db;
...
void apply_eosio_updateauth(apply_context& context) {
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "updateauth not allowed in read-only transaction" );
   ...
   CORE_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   CORE_ASSERT( update.permission.to_string().find( config::system_account_prefix_str() ) != 0,
               action_validate_exception,
               "Permission names that start with '${p}' are reserved",
               ("p", config::system_account_prefix_str()) );
   CORE_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   CORE_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      CORE_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      CORE_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      CORE_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      CORE_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      CORE_ASSERT(parent_id == permission->parent, action_validate_exception,
...
//   context.require_write_lock( config::auth_scope() );
   ...
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "deleteauth not allowed in read-only transaction" );
   ...
   CORE_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   CORE_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
...
//   context.require_write_lock( config::auth_scope() );
   ...
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "linkauth not allowed in read-only transaction" );

   auto requirement = context.get_action().data_as<linkauth>();
   try {
      CORE_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      CORE_ASSERT(account != nullptr, account_query_exception,
      ...
      CORE_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::any_name() ) {
         ...
         CORE_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         CORE_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
//   context.require_write_lock( config::auth_scope() );
   ...
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "unlinkauth not allowed in read-only transaction" );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   CORE_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
...
void apply_eosio_canceldelay(apply_context& context) {
   CORE_ASSERT( !context.trx_context.is_read_only(), action_validate_exception, "canceldelay not allowed in read-only transaction" );
