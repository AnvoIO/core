...
static constexpr uint64_t header_id = 0x434f4d5645524f43ULL; //"COREVMOC" little endian
static constexpr uint64_t legacy_header_id = 0x32434f4d56534f45ULL; //"EOSVMOC2" little endian (for migrating chains)
...
code_cache_async::code_cache_async(const std::filesystem::path& data_dir, const corevmoc::config& corevmoc_config,
                                   const chainbase::database& db, compile_complete_callback cb) :
   code_cache_base(data_dir, corevmoc_config, db),
   _compile_complete_func(std::move(cb)),
   _result_queue(corevmoc_config.threads * 2),
   _threads(corevmoc_config.threads)
{
   FC_ASSERT(_threads, "Core VM OC requires at least 1 compile thread");
      ...
      if(_outstanding_compiles_and_poison[result.code.code_id] == false) {
         std::visit(overloaded {
            ...
            [&](const compilation_result_unknownfailure&) {
               wlog("code ${c} failed to tier-up with Core VM OC", ("c", result.code.code_id));
...
code_cache_sync::~code_cache_sync() {
   ...
   auto [success, message, fds] = read_message_with_fds(_compile_monitor_read_socket);
   if(success)
      elog("unexpected response from Core VM OC compile monitor during shutdown");
}
...
code_cache_base::code_cache_base(const std::filesystem::path& data_dir, const corevmoc::config& corevmoc_config, const chainbase::database& db) :
   _db(db),
   _corevmoc_config(corevmoc_config),
   ...
   auto create_code_cache_file = [&] {
      ...
      new (creation_region.get_address()) allocator_t(corevmoc_config.cache_size, total_header_size);
   ...
   if(corevmoc_config.cache_size > existing_file_size) {
      std::filesystem::resize_file(_cache_file_path, corevmoc_config.cache_size);
      ...
      resize_allocator->grow(corevmoc_config.cache_size - existing_file_size);
   }
   ...
   if(cache_header.serialized_descriptor_index) {
      fc::datastream<const char*> ds(code_mapping + cache_header.serialized_descriptor_index, corevmoc_config.cache_size - cache_header.serialized_descriptor_index);
   ...
   munmap(code_mapping, corevmoc_config.cache_size);

   _free_bytes_eviction_threshold = corevmoc_config.cache_size * .1;
