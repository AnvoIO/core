...
void run_compile_trampoline(int fd) {
   ...
   while(true) {
      ...
      if(!std::holds_alternative<compile_wasm_message>(message) || fds.size() != 2) {
         std::cerr << "Core VM OC compile trampoline got unexpected message; ignoring" << std::endl;
         continue;
      }
      ...
      if(pid == 0) {
         ...
         std::cerr << "Core VM OC compile trampoline failed to spawn compile task" << std::endl;
   }
