...
#include <core_net/chain/webassembly/core-vm-oc/intrinsic_mapping.hpp>
...
#error Core VM OC is not compatible with Clang ShadowCallStack
#endif
#endif
...
static void segv_handler(int sig, siginfo_t* info, void* ctx)  {
...
#else
   uint64_t current_gs = core_vm_oc_getgs();
#endif
   if(current_gs == 0)
      goto notus;
   ...
   if((uintptr_t)info->si_addr >= cb_in_main_segment->execution_thread_memory_start &&
      (uintptr_t)info->si_addr < cb_in_main_segment->execution_thread_memory_start+cb_in_main_segment->execution_thread_memory_length)
         siglongjmp(*cb_in_main_segment->jmp, COREVMOC_EXIT_SEGV);
...
static intrinsic grow_memory_intrinsic COREVMOC_INTRINSIC_INIT_PRIORITY("corevmoc_internal.grow_memory", IR::FunctionType::get(IR::ResultType::i32,{IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&core_vm_oc_grow_memory,
  std::integral_constant<std::size_t, find_intrinsic_index("corevmoc_internal.grow_memory")>::value
);

//This is effectively overriding the core_net_exit intrinsic in wasm_interface
static void core_net_exit(int32_t code) {
   siglongjmp(*core_vm_oc_get_jmp_buf(), COREVMOC_EXIT_CLEAN_EXIT);
   __builtin_unreachable();
}
...
static void throw_internal_exception(const E& e) {
   *reinterpret_cast<std::exception_ptr*>(core_vm_oc_get_exception_ptr()) = std::make_exception_ptr(e);
   siglongjmp(*core_vm_oc_get_jmp_buf(), COREVMOC_EXIT_EXCEPTION);
   __builtin_unreachable();
}

#define DEFINE_COREVMOC_TRAP_INTRINSIC(module,name) \
	void name(); \
	static intrinsic name##Function COREVMOC_INTRINSIC_INIT_PRIORITY(#module "." #name,IR::FunctionType::get(),(void*)&name, \
     std::integral_constant<std::size_t, find_intrinsic_index(#module "." #name)>::value \
   ); \
	void name()

DEFINE_COREVMOC_TRAP_INTRINSIC(corevmoc_internal,depth_assert) {
   throw_internal_exception(wasm_execution_error(FC_LOG_MESSAGE(error, "Exceeded call depth maximum")));
}

DEFINE_COREVMOC_TRAP_INTRINSIC(corevmoc_internal,div0_or_overflow) {
   throw_internal_exception(wasm_execution_error(FC_LOG_MESSAGE(error, "Division by 0 or integer overflow trapped")));
}

DEFINE_COREVMOC_TRAP_INTRINSIC(corevmoc_internal,indirect_call_mismatch) {
   throw_internal_exception(wasm_execution_error(FC_LOG_MESSAGE(error, "Indirect call function type mismatch")));
}

DEFINE_COREVMOC_TRAP_INTRINSIC(corevmoc_internal,indirect_call_oob) {
   throw_internal_exception(wasm_execution_error(FC_LOG_MESSAGE(error, "Indirect call index out of bounds")));
}

DEFINE_COREVMOC_TRAP_INTRINSIC(corevmoc_internal,unreachable) {
   throw_internal_exception(wasm_execution_error(FC_LOG_MESSAGE(error, "Unreachable reached")));
}

static void core_vm_oc_check_memcpy_params(int32_t dest, int32_t src, int32_t length) {
...
static intrinsic check_memcpy_params_intrinsic("corevmoc_internal.check_memcpy_params", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32,IR::ValueType::i32,IR::ValueType::i32}),
  (void*)&core_vm_oc_check_memcpy_params,
  std::integral_constant<std::size_t, find_intrinsic_index("corevmoc_internal.check_memcpy_params")>::value
);
...
void executor::execute(const code_descriptor& code, memory& mem, apply_context& context) {
   ...
   uint64_t max_call_depth = core_net::chain::wasm_constraints::maximum_call_depth+1;
   uint64_t max_pages = core_net::chain::wasm_constraints::maximum_linear_memory/core_net::chain::wasm_constraints::wasm_page_size;
   ...
   if(code.starting_memory_pages > 0 ) {
      ...
      if(initial_page_offset < static_cast<uint64_t>(code.starting_memory_pages)) {
         mprotect(mem.full_page_memory_base() + initial_page_offset * core_net::chain::wasm_constraints::wasm_page_size,
                  (code.starting_memory_pages - initial_page_offset) * core_net::chain::wasm_constraints::wasm_page_size, PROT_READ | PROT_WRITE);
      }
      core_vm_oc_setgs((uint64_t)mem.zero_page_memory_base()+initial_page_offset*memory::stride);
      memset(mem.full_page_memory_base(), 0, 64u*1024u*code.starting_memory_pages);
   }
   else
      core_vm_oc_setgs((uint64_t)mem.zero_page_memory_base());
   ...
   auto cleanup = fc::make_scoped_exit([cb, &tt=context.trx_context.transaction_timer, &mem=mem](){
      ...
      if(cb->current_linear_memory_pages > base_pages) {
         mprotect(mem.full_page_memory_base() + base_pages * core_net::chain::wasm_constraints::wasm_page_size,
                  (cb->current_linear_memory_pages - base_pages) * core_net::chain::wasm_constraints::wasm_page_size, PROT_NONE);
      }
   });
   ...
   switch(sigsetjmp(*cb->jmp, 0)) {
      ...
      case COREVMOC_EXIT_CHECKTIME_FAIL:
