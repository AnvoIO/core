...
#define COREVMOC_MEMORY_PTR_cb_ptr GS_PTR struct core_vm_oc_control_block* const cb_ptr = ((GS_PTR struct core_vm_oc_control_block* const)(CORE_NET_VM_OC_CONTROL_BLOCK_OFFSET));
...
#define COREVMOC_MEMORY_PTR_cb_ptr \
   struct core_vm_oc_control_block* const cb_ptr = \
      (struct core_vm_oc_control_block*)((char*)core_vm_oc_getgs() + CORE_NET_VM_OC_CONTROL_BLOCK_OFFSET);

#endif

int32_t core_vm_oc_grow_memory(int32_t grow, int32_t max) {
   COREVMOC_MEMORY_PTR_cb_ptr;
   ...
   int64_t max_segments = cb_ptr->execution_thread_memory_length / CORE_NET_VM_OC_MEMORY_STRIDE - 1;
   ...
   current_gs += gs_diff * CORE_NET_VM_OC_MEMORY_STRIDE;
   core_vm_oc_setgs(current_gs);
...
sigjmp_buf* core_vm_oc_get_jmp_buf() {
   COREVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->jmp;
}

void* core_vm_oc_get_exception_ptr() {
   COREVMOC_MEMORY_PTR_cb_ptr;
   return cb_ptr->eptr;
}

void* core_vm_oc_get_bounce_buffer_list() {
   COREVMOC_MEMORY_PTR_cb_ptr;
...
uint64_t core_vm_oc_getgs_syscall() {
   uint64_t gs;
   arch_prctl(ARCH_GET_GS, &gs);
   return gs;
}

uint64_t __attribute__ ((__target__ ("fsgsbase"))) core_vm_oc_getgs_fsgsbase() {
   return _readgsbase_u64();
}

void core_vm_oc_setgs_syscall(uint64_t gs) {
   arch_prctl(ARCH_SET_GS, (unsigned long*)gs); //cast to a (unsigned long*) to match local declaration above
}

void __attribute__ ((__target__ ("fsgsbase"))) core_vm_oc_setgs_fsgsbase(uint64_t gs) {
   return _writegsbase_u64(gs);
}

extern char** _dl_argv;
static int core_vm_oc_use_fsgsbase() {
...
uint64_t (*resolve_core_vm_oc_getgs())() {
   if(core_vm_oc_use_fsgsbase())
      return core_vm_oc_getgs_fsgsbase;
   return core_vm_oc_getgs_syscall;
}

uint64_t core_vm_oc_getgs() __attribute__ ((ifunc ("resolve_core_vm_oc_getgs")));

void (*resolve_core_vm_oc_setgs())(uint64_t) {
   if(core_vm_oc_use_fsgsbase())
      return core_vm_oc_setgs_fsgsbase;
   return core_vm_oc_setgs_syscall;
}

void core_vm_oc_setgs(uint64_t) __attribute__ ((ifunc ("resolve_core_vm_oc_setgs")));
...
uint64_t core_vm_oc_getgs() {
   uint64_t val;
   __asm__ volatile("mov %0, x28" : "=r"(val));
   return val;
}

void core_vm_oc_setgs(uint64_t val) {
