...
namespace core_net { namespace chain { namespace webassembly {
   ...
   void interface::activate_feature( int64_t feature_name ) const {
      CORE_ASSERT( false, unsupported_feature, "Unsupported Hardfork Detected" );
   }

   void interface::preactivate_feature( legacy_ptr<const digest_type> feature_digest ) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "preactivate_feature not allowed in a readonly transaction");
   ...
   void interface::set_resource_limits( account_name account, int64_t ram_bytes, int64_t net_weight, int64_t cpu_weight ) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_resource_limits not allowed in a readonly transaction");
      CORE_ASSERT(ram_bytes >= -1, wasm_execution_error, "invalid value for ram resource limit expected [-1,INT64_MAX]");
      CORE_ASSERT(net_weight >= -1, wasm_execution_error, "invalid value for net resource weight expected [-1,INT64_MAX]");
      CORE_ASSERT(cpu_weight >= -1, wasm_execution_error, "invalid value for cpu resource weight expected [-1,INT64_MAX]");
   ...
   int64_t set_proposed_producers_common( apply_context& context, vector<producer_authority> && producers, bool validate_keys ) {
      ...
      for (const auto& p: producers) {
         CORE_ASSERT( context.is_account(p.producer_name), wasm_execution_error, "producer schedule includes a nonexisting account" );
         std::visit([&p, num_supported_key_types, validate_keys](const auto& a) {
            ...
            for (const auto& kw: a.keys ) {
               CORE_ASSERT( kw.key.which() < num_supported_key_types, unactivated_key_type,
                           "Unactivated key type used in proposed producer schedule");

               if( validate_keys ) {
                  CORE_ASSERT( kw.key.valid(), wasm_execution_error, "producer schedule includes an invalid key" );
               }
            ...
            CORE_ASSERT( a.keys.size() == unique_keys.size(), wasm_execution_error, "producer schedule includes a duplicated key for ${account}", ("account", p.producer_name));
            CORE_ASSERT( a.threshold > 0, wasm_execution_error, "producer schedule includes an authority with a threshold of 0 for ${account}", ("account", p.producer_name));
            CORE_ASSERT( sum_weights >= a.threshold, wasm_execution_error, "producer schedule includes an unsatisfiable authority for ${account}", ("account", p.producer_name));
         }, p.authority);

         unique_producers.insert(p.producer_name);
      }
      CORE_ASSERT( producers.size() == unique_producers.size(), wasm_execution_error, "duplicate producer name in producer schedule" );
   ...
   void interface::set_wasm_parameters_packed( span<const char> packed_parameters ) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_wasm_parameters_packed not allowed in a readonly transaction");
      fc::datastream<const char*> ds( packed_parameters.data(), packed_parameters.size() );
      uint32_t version;
      chain::wasm_config cfg;
      fc::raw::unpack(ds, version);
      CORE_ASSERT(version == 0, wasm_config_unknown_version, "set_wasm_parameters_packed: Unknown version: ${version}", ("version", version));
   ...
   int64_t interface::set_proposed_producers( legacy_span<const char> packed_producer_schedule) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_proposed_producers not allowed in a readonly transaction");
   ...
   int64_t interface::set_proposed_producers_ex( uint64_t packed_producer_format, legacy_span<const char> packed_producer_schedule) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_proposed_producers_ex not allowed in a readonly transaction");
      if (packed_producer_format == 0) {
         ...
         CORE_THROW(wasm_execution_error, "Producer schedule is in an unknown format!");
      }
   }
   ...
   void interface::set_finalizers(uint64_t packed_finalizer_format, span<const char> packed_finalizer_policy) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error,
                 "set_finalizers not allowed in a readonly transaction");
      if (packed_finalizer_format != 0) {
         CORE_THROW(wasm_execution_error, "Finalizer policy is in an unknown format!");
      }
      ...
      CORE_ASSERT( finalizers.size() <= config::max_finalizers, wasm_execution_error,
                  "Finalizer policy exceeds the maximum finalizer count for this chain" );
      CORE_ASSERT( finalizers.size() > 0, wasm_execution_error, "Finalizers cannot be empty" );
      ...
      for (auto& f: finalizers) {
         CORE_ASSERT( f.description.size() <= config::max_finalizer_description_size, wasm_execution_error,
                     "Finalizer description greater than ${s}", ("s", config::max_finalizer_description_size) );
         CORE_ASSERT(std::numeric_limits<uint64_t>::max() - weight_sum >= f.weight, wasm_execution_error,
                    "sum of weights causes uint64_t overflow");
         weight_sum += f.weight;
         CORE_ASSERT(f.public_key.size() == 96, wasm_execution_error, "Invalid bls public key length");
         fc::crypto::blslib::bls_public_key pk(std::span<const uint8_t,96>(f.public_key.data(), 96));
         CORE_ASSERT( unique_finalizer_keys.insert(pk).second, wasm_execution_error,
      ...
      CORE_ASSERT( weight_sum >= finpol.threshold && finpol.threshold > weight_sum / 2, wasm_execution_error,
   ...
   void interface::set_blockchain_parameters_packed( legacy_span<const char> packed_blockchain_parameters ) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_blockchain_parameters_packed not allowed in a readonly transaction");
   ...
   uint32_t interface::get_parameters_packed( span<const char> packed_parameter_ids, span<char> packed_parameters) const{
      ...
      CORE_ASSERT(size <= packed_parameters.size(),
   ...
   void interface::set_parameters_packed( span<const char> packed_parameters ){
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_parameters_packed not allowed in a readonly transaction");
      fc::datastream<const char*> ds( packed_parameters.data(), packed_parameters.size() );
   ...
   void interface::set_privileged( account_name n, bool is_priv ) {
      CORE_ASSERT(!context.trx_context.is_read_only(), wasm_execution_error, "set_privileged not allowed in a readonly transaction");
