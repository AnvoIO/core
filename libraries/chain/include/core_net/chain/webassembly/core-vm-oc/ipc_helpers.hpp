...
#include <core_net/chain/webassembly/core-vm-oc/memfd_helpers.hpp>
...
std::tuple<bool, corevmoc_message, std::vector<wrapped_fd>> read_message_with_fds(boost::asio::local::datagram_protocol::socket& s);
std::tuple<bool, corevmoc_message, std::vector<wrapped_fd>> read_message_with_fds(int fd);
bool write_message_with_fds(boost::asio::local::datagram_protocol::socket& s, const corevmoc_message& message, std::span<wrapped_fd> fds = {});
bool write_message_with_fds(int fd_to_send_to, const corevmoc_message& message, std::span<wrapped_fd> fds = {});

template<typename T>
wrapped_fd memfd_for_bytearray(const T& bytes) {
   int fd = exec_sealed_memfd_create("corevmoc_code");
