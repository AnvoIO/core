...
class memory {
      ...
      static_assert(sizeof(control_block) <= wcb_allowance, "Core VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(CORE_NET_VM_OC_CONTROL_BLOCK_OFFSET + cb_offset == 0, "Core VM OC control block offset has slid out of place somehow");
      static_assert(stride == CORE_NET_VM_OC_MEMORY_STRIDE, "Core VM OC memory stride has slid out of place somehow");
...
#define OFFSET_OF_CONTROL_BLOCK_MEMBER(M) (-(int)core_net::chain::corevmoc::memory::cb_offset + (int)offsetof(core_net::chain::corevmoc::control_block, M))
#define OFFSET_OF_FIRST_INTRINSIC ((int)-core_net::chain::corevmoc::memory::first_intrinsic_offset)
