...
#include <core_net/chain/webassembly/core-vm-oc/core-vm-oc.hpp>
...
#define COREVMOC_CB_FROM_BASE_REG() \
   reinterpret_cast<core_net::chain::corevmoc::control_block*>((char*)core_vm_oc_getgs() + CORE_NET_VM_OC_CONTROL_BLOCK_OFFSET)
#endif

using namespace IR;
using namespace fc;

using namespace core_net::chain::corevmoc;

class corevmoc_instantiated_module;

class corevmoc_runtime : public core_net::chain::wasm_runtime_interface {
   public:
      corevmoc_runtime(const std::filesystem::path data_dir, const corevmoc::config& corevmoc_config, const chainbase::database& db);
      ~corevmoc_runtime();
      ...
      friend corevmoc_instantiated_module;
      corevmoc::code_cache_sync cc;
      corevmoc::executor exec;
      corevmoc::memory mem;

      // Defined in core-vm-oc.cpp. Used for non-main thread in multi-threaded execution
      thread_local static std::unique_ptr<corevmoc::executor> exec_thread_local;
      thread_local static std::unique_ptr<corevmoc::memory> mem_thread_local;
};
...
inline void* array_ptr_impl (size_t ptr, size_t length)
{
   ...
   {
      auto* cb = COREVMOC_CB_FROM_BASE_REG();
...
#elif defined(__aarch64__)
   {
      char* base = (char*)core_vm_oc_getgs();
      auto* cb = COREVMOC_CB_FROM_BASE_REG();
...
#elif defined(__aarch64__)
   full_linear_memory_start = COREVMOC_CB_FROM_BASE_REG()->full_linear_memory_start;
#endif
...
struct core_vm_oc_execution_interface {
   inline const auto& operand_from_back(std::size_t index) const { return *(os - index - 1); }
   core_net::vm::native_value* os;
};

struct core_vm_oc_type_converter : public core_net::vm::type_converter<webassembly::interface, core_vm_oc_execution_interface> {
   using base_type = core_net::vm::type_converter<webassembly::interface, core_vm_oc_execution_interface>;
...
template<typename Args, std::size_t... Is>
auto get_ct_args(std::index_sequence<Is...>) {
   return std::tuple_cat(get_ct_args_i<core_vm_oc_type_converter, std::tuple_element_t<Is, Args>>()...);
}

struct result_resolver {
   // Suppress "expression result unused" warnings
   result_resolver(core_vm_oc_type_converter& tc) : tc(tc) {}
   template<typename T>
   auto operator,(T&& res) {
      return make_native_type(vm::detail::resolve_result(tc, static_cast<T&&>(res)));
   }
   core_vm_oc_type_converter& tc;
};
...
auto fn(A... a) {
   try {
      if constexpr(!is_injected) {
         ...
         constexpr int depth_assertion_intrinsic_offset = OFFSET_OF_FIRST_INTRINSIC - (int) find_intrinsic_index("corevmoc_internal.depth_assert") * 8;
...
#pragma GCC diagnostic pop
...
#elif defined(__aarch64__)
      ctx = COREVMOC_CB_FROM_BASE_REG()->ctx;
#endif
      Interface host(*ctx);
      core_vm_oc_type_converter tc{&host, core_vm_oc_execution_interface{stack + sizeof...(A)}};
      ...
      *reinterpret_cast<std::exception_ptr*>(core_vm_oc_get_exception_ptr()) = std::current_exception();
   }
...
} } } }// core_net::chain::webassembly::corevmoc
