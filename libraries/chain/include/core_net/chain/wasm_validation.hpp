...
namespace core_net { namespace chain { namespace wasm_validations {
   ...
   class wasm_binary_validation {
      ...
      public:
         wasm_binary_validation( const core_net::chain::controller& control, IR::Module& mod ) : _module( &mod ) {
         ...
         void validate() {
            ...
            for ( auto& fd : _module->functions.defs ) {
               wasm_ops::CORE_NET_OperatorDecoderStream<op_constrainers> decoder(fd.code);
               while ( decoder ) {
