...
namespace core_net { namespace chain {
   ...
   struct action : public action_base {
      ...
      template<typename T>
      T data_as()const {
         CORE_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         CORE_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
