...
/* clang-format off */
#define CORE_NET_VM_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define CORE_NET_VM_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define CORE_NET_VM_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define CORE_NET_VM_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define CORE_NET_VM_CALL_IMM_OPS(opcode_macro)       \
...
#define CORE_NET_VM_PARAMETRIC_OPS(opcode_macro)     \
...
#define CORE_NET_VM_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define CORE_NET_VM_MEMORY_OPS(opcode_macro)         \
...
#define CORE_NET_VM_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define CORE_NET_VM_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define CORE_NET_VM_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define CORE_NET_VM_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define CORE_NET_VM_COMPARISON_OPS(opcode_macro)     \
...
#define CORE_NET_VM_NUMERIC_OPS(opcode_macro)        \
...
#define CORE_NET_VM_CONVERSION_OPS(opcode_macro)     \
...
#define CORE_NET_VM_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define CORE_NET_VM_EMPTY_OPS(opcode_macro)          \
...
#define CORE_NET_VM_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define CORE_NET_VM_CREATE_ENUM(name, code) name = code,

#define CORE_NET_VM_CREATE_STRINGS(name, code) #name,

#define CORE_NET_VM_CREATE_MAP(name, code) { code, #name },

#define CORE_NET_VM_OPCODE_NAME_if_
#define CORE_NET_VM_OPCODE_NAME_else_
#define CORE_NET_VM_OPCODE_NAME_return_

#define CORE_NET_VM_OPCODE_NAME_TEST() 1
#define CORE_NET_VM_OPCODE_NAME_TEST_CORE_NET_VM_OPCODE_NAME_TEST 0,
#define CORE_NET_VM_OPCODE_NAME_TEST_1 1, ignore
#define CORE_NET_VM_EXPAND(x) x
#define CORE_NET_VM_CAT2(x, y) x ## y
#define CORE_NET_VM_CAT(x, y) CORE_NET_VM_CAT2(x, y)
#define CORE_NET_VM_APPLY(f, args) f args
#define CORE_NET_VM_FIX_OPCODE_NAME_0(name) name ## _t
#define CORE_NET_VM_FIX_OPCODE_NAME_1(name) name ## t
#define CORE_NET_VM_FIX_OPCODE_NAME(iskeyword, garbage) CORE_NET_VM_FIX_OPCODE_NAME_ ## iskeyword

#define CORE_NET_VM_OPCODE_T(name)                                                             \
   CORE_NET_VM_APPLY(CORE_NET_VM_FIX_OPCODE_NAME,                                                   \
      (CORE_NET_VM_CAT(CORE_NET_VM_OPCODE_NAME_TEST_,                                               \
           CORE_NET_VM_EXPAND(CORE_NET_VM_OPCODE_NAME_TEST CORE_NET_VM_OPCODE_NAME_ ## name ()))))(name)

#define CORE_NET_VM_CREATE_EXIT_TYPE(name, code)                                                                      \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define CORE_NET_VM_CREATE_CONTROL_FLOW_TYPES(name, code)                                                                   \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      CORE_NET_VM_OPCODE_T(name)() {}                                                                                       \
      CORE_NET_VM_OPCODE_T(name)(uint32_t data) : data(data) {}                                                             \
      CORE_NET_VM_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                                          \
...
#define CORE_NET_VM_CREATE_BR_TABLE_TYPE(name, code)                                                                        \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
...
#define CORE_NET_VM_CREATE_TYPES(name, code)                                                                                \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define CORE_NET_VM_CREATE_CALL_TYPES(name, code)                                                                           \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define CORE_NET_VM_CREATE_CALL_IMM_TYPES(name, code)                                                                       \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define CORE_NET_VM_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                                                \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define CORE_NET_VM_CREATE_MEMORY_TYPES(name, code)                                                                         \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
...
#define CORE_NET_VM_CREATE_I32_CONSTANT_TYPE(name, code)                                                                    \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      CORE_NET_VM_OPCODE_T(name)() = default;                                                                               \
      explicit CORE_NET_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit CORE_NET_VM_OPCODE_T(name)(int32_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         int32_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define CORE_NET_VM_CREATE_I64_CONSTANT_TYPE(name, code)                                                                    \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      CORE_NET_VM_OPCODE_T(name)() = default;                                                                               \
      explicit CORE_NET_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit CORE_NET_VM_OPCODE_T(name)(int64_t n) { data.i = n; }                                                        \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         int64_t  i;                                                                                                   \
      } data;                                                                                                          \
...
#define CORE_NET_VM_CREATE_F32_CONSTANT_TYPE(name, code)                                                                    \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      CORE_NET_VM_OPCODE_T(name)() = default;                                                                               \
      explicit CORE_NET_VM_OPCODE_T(name)(uint32_t n) { data.ui = n; }                                                      \
      explicit CORE_NET_VM_OPCODE_T(name)(float n) { data.f = n; }                                                          \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
         float    f;                                                                                                   \
      } data;                                                                                                          \
...
#define CORE_NET_VM_CREATE_F64_CONSTANT_TYPE(name, code)                                                                    \
   struct CORE_NET_VM_OPCODE_T(name) {                                                                                      \
      CORE_NET_VM_OPCODE_T(name)() = default;                                                                               \
      explicit CORE_NET_VM_OPCODE_T(name)(uint64_t n) { data.ui = n; }                                                      \
      explicit CORE_NET_VM_OPCODE_T(name)(double n) { data.f = n; }                                                         \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
         double   f;                                                                                                   \
      } data;                                                                                                          \
...
#define CORE_NET_VM_IDENTITY(name, code) core_net::vm::CORE_NET_VM_OPCODE_T(name),
#define CORE_NET_VM_IDENTITY_END(name, code) core_net::vm::CORE_NET_VM_OPCODE_T(name)
