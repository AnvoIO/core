...
#include <core_net/vm/options.hpp>
#include <core_net/vm/sections.hpp>
#include <core_net/vm/types.hpp>
#include <core_net/vm/utils.hpp>
#include <core_net/vm/vector.hpp>
#include <core_net/vm/debug_info.hpp>
...
namespace core_net { namespace vm {
   ...
   struct max_mutable_globals_checker<Options, std::void_t<max_mutable_globals_t<Options>>> {
      ...
      void on_mutable_global(const Options& options, uint8_t type) {
         ...
         _counter += size;
         CORE_NET_VM_ASSERT(_counter <= options.max_mutable_global_bytes && _counter >= size, wasm_parse_exception, "mutable globals exceeded limit");
      }
...
#define MAX_ELEMENTS(name, default_)\
   ...
   struct max_func_local_bytes_checker<Options, std::void_t<decltype(std::declval<Options>().max_func_local_bytes)>> {
      ...
      void on_type(const Options& options, std::uint8_t type) {
         unsigned size = get_size_for_type(type);
         _count += size;
         CORE_NET_VM_ASSERT(_count <= options.max_func_local_bytes && _count >= size, wasm_parse_exception, "local variable limit exceeded");
      }
   ...
   // Matches the behavior of core_net::chain::wasm_validations::nested_validator
   template<typename Options, typename Enable = void>
   struct core_net_max_nested_structures_checker {
      void on_control(const Options&) {}
      void on_end(const Options&) {}
   };
   template<typename Options>
   struct core_net_max_nested_structures_checker<Options, std::void_t<decltype(std::declval<Options>().core_net_max_nested_structures)>> {
      void on_control(const Options& options) {
         ++_count;
         CORE_NET_VM_ASSERT(_count <= options.core_net_max_nested_structures, wasm_parse_exception, "Nested depth exceeded");
      }
      void on_end(const Options& options) {
         if(_count == 0) ++_count;
         else --_count;
      }
      std::decay_t<decltype(std::declval<Options>().core_net_max_nested_structures)> _count = 0;
   };
    ...
    public:
      ...
      int validate_utf8_code_point(wasm_code_ptr& code) {
         ...
         if (ch < 0x80) {
           ...
           CORE_NET_VM_ASSERT((ch & 0xC0) == 0xC0, wasm_parse_exception, "invalid utf8 encoding");
            unsigned char b2 = *code++;
            CORE_NET_VM_ASSERT((b2 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            uint32_t code_point =
              (static_cast<uint32_t>(ch - 0xC0u) << 6u) +
              (static_cast<uint32_t>(b2 - 0x80u));
            CORE_NET_VM_ASSERT(0x80 <= code_point && code_point < 0x800, wasm_parse_exception, "invalid utf8 encoding");
            return 2;
         } else if(ch < 0xF0) {
            unsigned char b2 = *code++;
            CORE_NET_VM_ASSERT((b2 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            unsigned char b3 = *code++;
            CORE_NET_VM_ASSERT((b3 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            uint32_t code_point =
              (static_cast<uint32_t>(ch - 0xE0u) << 12u) +
              (static_cast<uint32_t>(b2 - 0x80u) << 6u) +
              (static_cast<uint32_t>(b3 - 0x80u));
            CORE_NET_VM_ASSERT((0x800 <= code_point && code_point < 0xD800) ||
            ...
            CORE_NET_VM_ASSERT((b4 & 0xC0) == 0x80, wasm_parse_exception, "invalid utf8 encoding");
            uint32_t code_point =
            ...
            CORE_NET_VM_ASSERT((0x10000 <= code_point && code_point < 0x110000),
                          wasm_parse_exception, "invalid utf8 encoding");
            return 4;
         }
         CORE_NET_VM_ASSERT(false, wasm_parse_exception, "invalid utf8 encoding");
      }
      ...
      guarded_vector<uint8_t> parse_utf8_string(wasm_code_ptr& code, std::uint32_t max_size) {
         ...
         CORE_NET_VM_ASSERT(len <= max_size, wasm_parse_exception, "name too long");
      ...
      void parse_module(wasm_code_ptr& code_ptr, size_t sz, module& mod, DebugInfo& debug) {
         _mod = &mod;
         CORE_NET_VM_ASSERT(parse_magic(code_ptr) == constants::magic, wasm_parse_exception, "magic number did not match");
         CORE_NET_VM_ASSERT(parse_version(code_ptr) == constants::version, wasm_parse_exception,
         ...
         for (;;) {
            ...
            CORE_NET_VM_ASSERT(id == 0 || id > highest_section_id, wasm_parse_exception, "section out of order");
            ...
            switch (id) {
               ...
               default: CORE_NET_VM_ASSERT(false, wasm_parse_exception, "error invalid section id");
            }
         }
         CORE_NET_VM_ASSERT(_mod->code.size() == _mod->functions.size(), wasm_parse_exception, "code section must have the same size as the function section" );
      ...
      inline void parse_name_section(wasm_code_ptr& code) {
         ...
         if(code.bounds() == code.offset()) return;
         CORE_NET_VM_ASSERT(false, wasm_parse_exception, "Invalid subsection Id");
      }
      ...
      void parse_import_entry(wasm_code_ptr& code, import_entry& entry) {
         ...
         switch ((uint8_t)entry.kind) {
            case external_kind::Function:
               ...
               CORE_NET_VM_ASSERT(type < _mod->types.size(), wasm_parse_exception, "Invalid function type");
               break;
            default: CORE_NET_VM_ASSERT(false, wasm_unsupported_import_exception, "only function imports are supported");
         }
      }
      ...
      uint8_t parse_flags(wasm_code_ptr& code) {
         if (detail::get_allow_u32_limits_flags(_options)) {
            ...
            CORE_NET_VM_ASSERT(*code == 0x0 || *code == 0x1, wasm_parse_exception, "invalid flags");
            return *code++;
         }
      }
      ...
      void parse_table_type(wasm_code_ptr& code, table_type& tt) {
         ...
         CORE_NET_VM_ASSERT(tt.element_type == types::anyfunc, wasm_parse_exception, "table must have type anyfunc");
         tt.limits.flags   = parse_flags(code);
         tt.limits.initial = parse_varuint32(code);
         if (tt.limits.flags) {
            tt.limits.maximum = parse_varuint32(code);
            CORE_NET_VM_ASSERT(tt.limits.initial <= tt.limits.maximum, wasm_parse_exception, "table max size less than min size");
         }
         CORE_NET_VM_ASSERT(tt.limits.initial <= detail::get_max_table_elements(_options), wasm_parse_exception, "table size exceeds limit");
      ...
      void parse_global_variable(wasm_code_ptr& code, global_variable& gv) {
         ...
         CORE_NET_VM_ASSERT(ct == types::i32 || ct == types::i64 || ct == types::f32 || ct == types::f64,
      ...
      void parse_memory_type(wasm_code_ptr& code, memory_type& mt) {
         ...
         CORE_NET_VM_ASSERT(mt.limits.initial <= detail::get_max_pages(_options), wasm_parse_exception, "initial memory out of range");
         // WASM specification
         CORE_NET_VM_ASSERT(mt.limits.initial <= 65536u, wasm_parse_exception, "initial memory out of range");
         if (mt.limits.flags) {
            mt.limits.maximum = parse_varuint32(code);
            CORE_NET_VM_ASSERT(mt.limits.maximum >= mt.limits.initial, wasm_parse_exception, "maximum must be at least minimum");
            CORE_NET_VM_ASSERT(mt.limits.maximum <= 65536u, wasm_parse_exception, "maximum memory out of range");
         }
      }
      ...
      void parse_export_entry(wasm_code_ptr& code, export_entry& entry) {
         ...
         switch(entry.kind) {
            case external_kind::Function: CORE_NET_VM_ASSERT(entry.index < _mod->get_functions_total(), wasm_parse_exception, "function export out of range"); break;
            case external_kind::Table: CORE_NET_VM_ASSERT(entry.index < _mod->tables.size(), wasm_parse_exception, "table export out of range"); break;
            case external_kind::Memory: CORE_NET_VM_ASSERT(entry.index < _mod->memories.size(), wasm_parse_exception, "memory export out of range"); break;
            case external_kind::Global:
               CORE_NET_VM_ASSERT(entry.index < _mod->globals.size(), wasm_parse_exception, "global export out of range");
               CORE_NET_VM_ASSERT(!detail::get_forbid_export_mutable_globals(_options) || !_mod->globals.at(entry.index).type.mutability,
                             wasm_parse_exception, "cannot export mutable globals");
               break;
            default: CORE_NET_VM_ASSERT(false, wasm_parse_exception, "Unknown export kind"); break;
         }
      }

      void parse_func_type(wasm_code_ptr& code, func_type& ft) {
         ft.form                              = *code++;
         CORE_NET_VM_ASSERT(ft.form == 0x60, wasm_parse_exception, "invalid function type");
         decltype(ft.param_types) param_types = { _allocator, parse_varuint32(code) };
         for (size_t i = 0; i < param_types.size(); i++) {
            uint8_t pt        = *code++;
            param_types.at(i) = pt;
            CORE_NET_VM_ASSERT(pt == types::i32 || pt == types::i64 || pt == types::f32 || pt == types::f64,
                          wasm_parse_exception, "invalid function param type");
         }
         ft.param_types  = std::move(param_types);
         ft.return_count = *code++;
         CORE_NET_VM_ASSERT(ft.return_count < 2, wasm_parse_exception, "invalid function return count");
         if (ft.return_count > 0) {
            uint8_t rt        = *code++;
            ft.return_type = rt;
            CORE_NET_VM_ASSERT(rt == types::i32 || rt == types::i64 || rt == types::f32 || rt == types::f64,
      ...
      void parse_elem_segment(wasm_code_ptr& code, elem_segment& es) {
         ...
         CORE_NET_VM_ASSERT(tt != nullptr, wasm_parse_exception, "table not declared");
         es.index = parse_varuint32(code);
         CORE_NET_VM_ASSERT(es.index == 0, wasm_parse_exception, "only table index of 0 is supported");
         parse_init_expr(code, es.offset, types::i32);
         uint32_t           size  = parse_varuint32(code);
         CORE_NET_VM_ASSERT(size <= detail::get_max_element_segment_elements(_options), wasm_parse_exception, "elem segment too large");
         decltype(es.elems) elems = { _allocator, size };
         for (uint32_t i = 0; i < size; i++) {
            uint32_t index                     = parse_varuint32(code);
            elems.at(i)                        = index;
            CORE_NET_VM_ASSERT(index < _mod->get_functions_total(), wasm_parse_exception,  "elem for undefined function");
         }
      ...
      void parse_init_expr(wasm_code_ptr& code, init_expr& ie, uint8_t type) {
         ...
         switch (ie.opcode) {
            case opcodes::i32_const:
               ...
               CORE_NET_VM_ASSERT(type == types::i32, wasm_parse_exception, "expected i32 initializer");
               break;
            case opcodes::i64_const:
               ie.value.i64 = parse_varint64(code);
               CORE_NET_VM_ASSERT(type == types::i64, wasm_parse_exception, "expected i64 initializer");
               break;
            case opcodes::f32_const:
               ie.value.f32 = parse_raw<uint32_t>(code);
               CORE_NET_VM_ASSERT(type == types::f32, wasm_parse_exception, "expected f32 initializer");
               break;
            case opcodes::f64_const:
               ie.value.f64 = parse_raw<uint64_t>(code);
               CORE_NET_VM_ASSERT(type == types::f64, wasm_parse_exception, "expected f64 initializer");
               break;
            default:
               CORE_NET_VM_ASSERT(false, wasm_parse_exception,
                             "initializer expression can only acception i32.const, i64.const, f32.const and f64.const");
         }
         CORE_NET_VM_ASSERT((*code++) == opcodes::end, wasm_parse_exception, "no end op found");
      }

      void parse_function_body(wasm_code_ptr& code, function_body& fb, std::size_t idx) {
         fb.size   = parse_varuint32(code);
         CORE_NET_VM_ASSERT(fb.size <= detail::get_max_code_bytes(_options), wasm_parse_exception, "Function body too large");
         const auto&         before    = code.offset();
         const auto&         local_cnt = parse_varuint32(code);
         _current_function_index++;
         CORE_NET_VM_ASSERT(local_cnt <= detail::get_max_local_sets(_options), wasm_parse_exception, "Number of local sets exceeds limit");
         ...
         for (size_t i = 0; i < local_cnt; i++) {
            ...
            CORE_NET_VM_ASSERT(type == types::i32 || type == types::i64 || type == types::f32 || type == types::f64,
         ...
         code += fb.size-1;
         CORE_NET_VM_ASSERT(detail::get_allow_code_after_function_end(_options) || *code == 0x0B,
      ...
      struct operand_stack_type_tracker {
         ...
         void push(uint8_t type) {
            ...
            CORE_NET_VM_ASSERT(operand_depth < std::numeric_limits<uint32_t>::max(), wasm_parse_exception, "integer overflow in operand depth");
            ++operand_depth;
         ...
         void pop(uint8_t expected) {
            ...
            CORE_NET_VM_ASSERT(!state.empty(), wasm_parse_exception, "unexpected pop");
            if (state.back() != unreachable_tag) {
               CORE_NET_VM_ASSERT(state.back() == expected || state.back() == any_type, wasm_parse_exception, "wrong type");
         ...
         uint8_t pop() {
            CORE_NET_VM_ASSERT(!state.empty() && state.back() != scope_tag, wasm_parse_exception, "unexpected pop");
            if (state.back() == unreachable_tag)
               return any_type;
            else {
         ...
         void pop_scope(uint8_t expected_result = types::pseudo) {
            pop(expected_result);
            CORE_NET_VM_ASSERT(!state.empty(), wasm_parse_exception, "unexpected end");
            if (state.back() == unreachable_tag) {
               local_bytes_checker.pop_unreachable();
               state.pop_back();
            }
            CORE_NET_VM_ASSERT(state.back() == scope_tag, wasm_parse_exception, "unexpected end");
            state.pop_back();
         ...
         void finish() {
            ...
            CORE_NET_VM_ASSERT(state.empty(), wasm_parse_exception, "stack not empty at scope end");
         }
      ...
      struct local_types_t {
         local_types_t(const func_type& ft, const guarded_vector<local_entry>& locals_arg) :
            ...
            for (uint32_t i = 0; i < locals_arg.size(); ++i) {
               ...
               CORE_NET_VM_ASSERT (count <= 0xFFFFFFFFu - locals_arg[i].count, wasm_parse_exception, "too many locals");
               count += locals_arg[i].count;
               _boundaries.push_back(count);
            }
         }
         uint8_t operator[](uint32_t local_idx) const {
            CORE_NET_VM_ASSERT(local_idx < _boundaries.back(), wasm_parse_exception, "undefined local");
      ...
      void parse_function_body_code(wasm_code_ptr& code, size_t bounds, const detail::max_func_local_bytes_stack_checker<Options>& local_bytes_checker,
         ...
         auto handle_branch_target = [&](uint32_t label, branch_t address) {
            CORE_NET_VM_ASSERT(label < pc_stack.size(), wasm_parse_exception, "invalid label");
         ...
         auto exit_scope = [&]() {
            ...
            CORE_NET_VM_ASSERT(pc_stack.size(), wasm_parse_exception, "unexpected end instruction");
            // an if with an empty else cannot have a return value
            CORE_NET_VM_ASSERT(!pc_stack.back().is_if || pc_stack.back().expected_result == types::pseudo, wasm_parse_exception, "wrong type");
         ...
         auto check_in_bounds = [&]{
            CORE_NET_VM_ASSERT(!detail::get_allow_code_after_function_end(_options) || !pc_stack.empty(),
                          wasm_parse_exception, "code after function end");
         };

         while (code.offset() < bounds) {
            CORE_NET_VM_ASSERT(pc_stack.size() <= detail::get_max_nested_structures(_options), wasm_parse_exception,
            ...
            switch (*code++) {
               ...
               case opcodes::end: {
                  check_in_bounds();
                  exit_scope();
                  CORE_NET_VM_ASSERT(detail::get_allow_code_after_function_end(_options) ||
               ...
               case opcodes::block: {
                  ...
                  CORE_NET_VM_ASSERT(expected_result == types::i32 || expected_result == types::i64 ||
               ...
               case opcodes::else_: {
                  ...
                  CORE_NET_VM_ASSERT(old_index.is_if, wasm_parse_exception, "else outside if");
               ...
               case opcodes::br_table: {
                  ...
                  CORE_NET_VM_ASSERT(table_size <= detail::get_max_br_table_elements(_options), wasm_parse_exception, "Too many labels in br_table");
                  ...
                  for (size_t i = 0; i < table_size; i++) {
                     ...
                     if(i == 0) {
                        ...
                        CORE_NET_VM_ASSERT(result_type == one_result, wasm_parse_exception, "br_table labels must have the same type");
                     }
                  }
                  uint32_t label = parse_varuint32(code);
                  auto branch = handler.emit_default(compute_depth_change(label));
                  handle_branch_target(label, branch);
                  CORE_NET_VM_ASSERT(table_size == 0 || result_type == pc_stack[pc_stack.size() - label - 1].label_result,
               ...
               case opcodes::call: {
                  ...
                  CORE_NET_VM_ASSERT(ft.return_count <= 1, wasm_parse_exception, "unsupported");
                  if(ft.return_count)
               ...
               case opcodes::call_indirect: {
                  ...
                  CORE_NET_VM_ASSERT(_mod->tables.size() > 0, wasm_parse_exception, "call_indirect requires a table");
                  op_stack.pop(types::i32);
                  for(uint32_t i = 0; i < ft.param_types.size(); ++i)
                     op_stack.pop(ft.param_types[ft.param_types.size() - i - 1]);
                  CORE_NET_VM_ASSERT(ft.return_count <= 1, wasm_parse_exception, "unsupported");
                  ...
                  CORE_NET_VM_ASSERT(*code == 0, wasm_parse_exception, "call_indirect must end with 0x00.");
                  code++; // 0x00
                  break;
               }
               ...
               case opcodes::select: {
                  ...
                  CORE_NET_VM_ASSERT(t0 == t1 || t0 == any_type || t1 == any_type, wasm_parse_exception, "incorrect types for select");
               ...
               case opcodes::set_global: {
                  check_in_bounds();
                  uint32_t global_idx = parse_varuint32(code);
                  CORE_NET_VM_ASSERT(_mod->globals.at(global_idx).type.mutability, wasm_parse_exception, "cannot set const global");
...
#define LOAD_OP(op_name, max_align, type)                            \
               case opcodes::op_name: {                              \
                  check_in_bounds();                                 \
                  CORE_NET_VM_ASSERT(_mod->memories.size() > 0, wasm_parse_exception, "load requires memory"); \
                  uint32_t alignment = parse_varuint32(code);        \
                  uint32_t offset = parse_varuint32(code);           \
                  CORE_NET_VM_ASSERT(alignment <= uint32_t(max_align), wasm_parse_exception, "alignment cannot be greater than size."); \
                  CORE_NET_VM_ASSERT(offset <= detail::get_max_memory_offset(_options), wasm_parse_exception, "load offset too large."); \
                  op_stack.pop(types::i32);                          \
...
#define STORE_OP(op_name, max_align, type)                           \
               case opcodes::op_name: {                              \
                  check_in_bounds();                                 \
                  CORE_NET_VM_ASSERT(_mod->memories.size() > 0, wasm_parse_exception, "store requires memory"); \
                  uint32_t alignment = parse_varuint32(code);        \
                  uint32_t offset = parse_varuint32(code);           \
                  CORE_NET_VM_ASSERT(alignment <= uint32_t(max_align), wasm_parse_exception, "alignment cannot be greater than size."); \
                  CORE_NET_VM_ASSERT(offset <= detail::get_max_memory_offset(_options), wasm_parse_exception, "store offset too large."); \
...
#undef STORE_OP
               ...
               case opcodes::current_memory:
                  CORE_NET_VM_ASSERT(_mod->memories.size() != 0, wasm_parse_exception, "memory.size requires memory");
                  op_stack.push(types::i32);
                  CORE_NET_VM_ASSERT(*code == 0, wasm_parse_exception, "memory.size must end with 0x00");
                  code++;
                  code_writer.emit_current_memory();
                  break;
               case opcodes::grow_memory:
                  check_in_bounds();
                  CORE_NET_VM_ASSERT(_mod->memories.size() != 0, wasm_parse_exception, "memory.grow requires memory");
                  op_stack.pop(types::i32);
                  op_stack.push(types::i32);
                  CORE_NET_VM_ASSERT(*code == 0, wasm_parse_exception, "memory.grow must end with 0x00");
                  code++;
...
#undef UNOP
#undef BINOP
               default: CORE_NET_VM_ASSERT(false, wasm_parse_exception, "Illegal instruction");
            }
         }
         CORE_NET_VM_ASSERT( pc_stack.empty(), wasm_parse_exception, "function body too long" );
      ...
      void parse_data_segment(wasm_code_ptr& code, data_segment& ds) {
         CORE_NET_VM_ASSERT(_mod->memories.size() != 0, wasm_parse_exception, "data requires memory");
         ds.index = parse_varuint32(code);
         parse_init_expr(code, ds.offset, types::i32);
         auto len =  parse_varuint32(code);
         CORE_NET_VM_ASSERT(len <= detail::get_max_data_segment_bytes(_options), wasm_parse_exception, "data segment too large.");
         CORE_NET_VM_ASSERT(static_cast<uint64_t>(static_cast<uint32_t>(ds.offset.value.i32)) + len <= detail::get_max_linear_memory_init(_options),
      ...
      inline void parse_section_impl(wasm_code_ptr& code, vec<Elem>& elems, std::uint32_t max_elements, ParseFunc&& elem_parse) {
         auto count = parse_varuint32(code);
         CORE_NET_VM_ASSERT(count <= max_elements, wasm_parse_exception, "number of section elements exceeded limit");
      ...
      inline void parse_section(wasm_code_ptr&                                                                 code,
         ...
         parse_section_impl(code, elems, 1, [&](wasm_code_ptr& code, memory_type& mt, std::size_t idx) {
            CORE_NET_VM_ASSERT(idx == 0, wasm_parse_exception, "only one memory is permitted");
      ...
      inline void parse_section(wasm_code_ptr&                                                                 code,
         ...
         CORE_NET_VM_ASSERT( elems.size() == _mod->functions.size(), wasm_parse_exception, "code section must have the same size as the function section" );
      ...
      void validate_exports() const {
         ...
         CORE_NET_VM_ASSERT(it == export_names.end(), wasm_parse_exception, "duplicate export name");
      }
    ...
    private:
      ...
      detail::core_net_max_nested_structures_checker<Options> _nested_checker;
